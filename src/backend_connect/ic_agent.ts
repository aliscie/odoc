...
export const createActor = (canisterId: any, options = {}) => {
  const agent =
    (options as any).agent ||
    new HttpAgent({ ...(options as any).agentOptions });

  if ((options as any).agent && (options as any).agentOptions) {
    console.warn(
...
export const get_actor = async () => {
  await new Promise<void>((resolve) => !loading && resolve());
  ...
  if (!backendActor) {
    if (import.meta.env.VITE_USE_WALLET) {
      ...
      backendActor = await plug.createActor({
        canisterId,
