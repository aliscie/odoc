...
function App() {
    ...
    useEffect(() => {
        (async () => {
            ...
            const icUrl = `http://127.0.0.1:${process.env.VITE_DFX_PORT}`;
            ...
            if (await authClient.isAuthenticated() && !isWebSocketSetup.current) {
                ...
                //     let key = event.data.notification[0] && Object.keys(event.data.notification[0].content)[0];
                //
                //     if (!key) {
                //         return
                //     }
                ...
                //             new_friends && dispatch(handleRedux("UPDATE_FRIEND", {friends: await actor.get_friends()[0]}))
                //             break
                //         case 'ContractUpdate':
                ...
                //             break
                //         case 'ConformShare':
                //                 let update_contract = event.data.notification[0].content['ConformShare']
                //                 dispatch(handleRedux("UPDATE_CONTRACT", {contract: update_contract}))
                //             break
                //         default:
                //             console.error("from app.tsx no key found")
                //             break
                //     }
                //
                // };
