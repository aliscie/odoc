...
const Discover = () => {
  ...
  const { isLoggedIn, Anonymous } = useSelector(
  ...
  useEffect(() => {
    ...
    const delayedSearch = async () => {
      ...
      if (searchValue.length > 0) {
        let res: undefined | Array<PostUser> =
          await backendActor.search_posts(searchValue);
        res && setPosts(res);
      } else {
        setPage(0);
        setPosts([]);
  ...
  async function set_posts() {
    ...
    let res: undefined | Array<PostUser> = await backendActor.get_posts(
      BigInt(current_page),
