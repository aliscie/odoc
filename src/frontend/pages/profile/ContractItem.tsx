...
const ContractItem: React.FC<ContractItemProps> = ({
    ...
    const [users, setUsers] = useState<{ sender: string; receiver: string }>({ sender: "Null", receiver: "Null" });
    ...
    useEffect(() => {
        (async () => {
            ...
            const senderUser = sender === "ExternalWallet" ? { name: "ExternalWallet" } : await getUser(sender.toString());
            setUsers({
    ...
    const handleDelete = async () => {
        ...
        const loader = enqueueSnackbar(<><span>Deleting...</span><span className="loader"/></>);
    ...
    const DeleteDialog: React.FC<{ handleCancel: () => void; handleClick: () => void }> = ({ handleCancel, handleClick }) => (
        <>
