...
function useCreateChatGroup() {
  ...
  const [admins, setAdmins] = useState([
    {
      title: profile?.name || "",
      id: Principal.fromText(profile?.id || "2vxsx-fae"),
    },
  ]);
  ...
  const workspacesOptions = workspaces
    ? workspaces.map((w: WorkSpace) => w.name)
    : [];
  ...
  const topDialog = {
    ...
    handleSave: async () => {
      ...
      try {
        ...
        if (res && "Ok" in res) {
          enqueueSnackbar("Chat group created successfully", {
            variant: "success",
          });
      ...
      <>
        ...
        <MultiAutoComplete
          ...
          onChange={(event: any, values: any) => {
            setAdmins(
              values.some((v: any) => v.id.toText() === profile.id)
                ? values
                : [...values, admins[0]],
            );
          }}
