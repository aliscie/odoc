...
function CustomDataGrid(props: Props) {
  ...
  const handleProcessRowUpdateError = React.useCallback(
    (params: any) => {
      console.log(
        "An error occurred while updating the row with params:",
        params,
      );
  ...
  function CustomCell(props: any) {
    ...
    const add_row = [
      <Button onClick={() => handleAddRow(props.rowId, true)} key="two">
        <ArrowCircleUpIcon />
      </Button>,
      <Button onClick={() => handleAddRow(props.rowId, false)} key="three">
        <ArrowCircleDownIcon />
      </Button>,
      <span
        onClick={() => handleAddRow(props.rowId, false)}
        style={{ width: "100px" }}
        key="add-row"
      >
        Add row
    ...
    const add_column = [
      <Button
        ...
        onClick={() => handleAddColumn(props.column.field, true)}
        key="two"
      >
        <ArrowCircleLeftIcon />
      </Button>,
      <Button
        onClick={() => handleAddColumn(props.column.field, false)}
        key="three"
      >
    ...
    let options = [
      ...
      {
        content: (
          <ButtonGroup {...button_group_props}>{add_column}</ButtonGroup>
        ),
        // preventClose: true,
      },
    ...
    if (["receiver", "share"].includes(field)) {
      options = options.filter(
        (item: any) => !["Formula", "Delete column"].includes(item.content),
      );
    }

    let children = <GridCell {...props} />;

    return <ContextMenu options={options}>{children}</ContextMenu>;
  }
